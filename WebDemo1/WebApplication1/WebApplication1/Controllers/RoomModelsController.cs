...
namespace WebApplication1.Controllers
{
    public class RoomModelsController : Controller
    {
        ...
        public ActionResult Index()
        {
            ...
            var pageCount = GetPageCount(recordCount);
            room = room.Where(m => m.Page.Contains("内部"));
            //ViewBag.PageIndex = 0;
        ...
        public ActionResult Index2()
        {
            var room = db.RoomModels as IQueryable<RoomModels>;
            var recordCount = room.Count();
            room = room.Where(m => m.Page.Contains("坂田"));
            //return View(db.RoomModels.ToList());
            return View(GetPagedDataSource(room, 0, recordCount));
        }
        ...
        */
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Create([Bind(Include = "ID,Name,Peopel,Coustomer,StartTime,EndTime,Tips,Status,Page")] RoomModels roomModels)
        {
        ...
        [HttpPost]
        [ValidateAntiForgeryToken]
        public ActionResult Edit([Bind(Include = "ID,Name,Peopel,Coustomer,StartTime,EndTime,Tips,Status,Page")] RoomModels roomModels)
        {
            if (ModelState.IsValid)
            {
